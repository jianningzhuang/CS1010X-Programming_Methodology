...
# Time complexity of insert_tree: O(n) where n is the number of elements in the tree
# at each level, insert_tree checks if the tree we are inserting into is empty
# if not, it checks the left and right branch if x <= current node or x > current_node respectively
# hence, in the worst case, it checks through to the deepest node in the tree
# if the tree is horribly skewed (e.g every child is larger than parent => the tree only branches out on the right)
# if the tree has n elements, the depth of a skewed tree is n, hence worst time complexity is O(n) since comparing x and build_tree takes O(1) constant time at each level
# if the tree is balanced, time complexity would be O(log n)
...
# similar explanation as 2b
# worst case x not in tree => check through to deepest node
...
##    if is_empty_tree(tree):
##        return []
##    elif not is_empty_tree(left_branch(tree)):
##        return flatten(left_branch(tree)) + [entry(tree)] + flatten(right_branch(tree))
##    elif not is_empty_tree(right_branch(tree)):
...
def sort_it(lst):
    return flatten(accumulate(insert_tree, make_empty_tree(), lst))
    
    
##    sorted_tree = make_empty_tree()
##    for elem in lst:
##        sorted_tree = insert_tree(elem, sorted_tree)
...
# time complexity of making the tree from lst  = 1 + 2 + .... n = O(n^2)
# time complexity of flattening the tree  = O(n^2) as there are only n elements
# overall time complexity = O(n^2) + O(n^2) = O(n^2) as they are not nested
