...
##            self.threshold = threshold
    ...
    def go_to_heaven1(self):
        self.get_place().add_object(Food(self.name + " meat", self.food_value))
        super().go_to_heaven()
    def go_to_heaven(self):
        name = self.get_name() + "'s meat"
        location = self.get_place()
        food_value = self.get_food_value()
        new_food = Food(name, food_value)
        new_food.place = location
        new_food.get_place().add_object(new_food) #adds itself to Place
        super().go_to_heaven()
...
def test_task5():
    ...
    deer1 = Animal('deer1', 15, 6)
    print(deer1.get_threshold())

    deer2 = Animal('deer2', 15, 6)
    print(deer2.get_threshold())

    deer3 = Animal('deer3', 15, 6)
    print(deer3.get_threshold())
